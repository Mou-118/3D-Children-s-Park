...
void bed(unsigned int& cubeVAO, Shader& lightingShader, glm::mat4 alTogether)
{
    ...
    drawCube(cubeVAO, lightingShader, model, 0.0, 1.0, 0.0); //this model is later sent to the vertex shader. //altogether is a matrix that is multiplied with every parts of the bed, so everything is part of Altogether. when rotated, everything is rotated AllTogether
    //color
//foam
    model = glm::mat4(1.0f);
